<x-user-layout>
    {{-- 1. 警告ヘッダー --}}
    {{-- 1. 安全ガイドヘッダー --}}
    <div class="bg-blue-50 border-b border-blue-100 py-3 px-4 flex items-center gap-3">
        <div class="flex-shrink-0 bg-blue-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs">
            <span>⚠️</span>
        </div>
        <div class="flex-1">
            <p class="text-blue-900 text-[13px] font-bold leading-tight">より安全な交換のために</p>
            <p class="text-blue-700 text-[11px] leading-tight">トラブル防止のため、交換場所は駅や公園などの公共施設を推奨しています</p>
        </div>
    </div>
    ...
    <div class="max-w-2xl mx-auto min-h-screen bg-white flex flex-col border-x border-gray-100 shadow-sm">
        ...
        <div class="p-4 border-t bg-white sticky bottom-0 z-20">
            {{-- メッセージ送信 --}}
            <form method="post" action="{{ route('create_message') }}" class="flex items-center gap-2 mb-4">
                @csrf
                <input type="hidden" name="thread_id" value="{{ $thread_id }}">

                <input type="text" name="message_text" placeholder="メッセージを入力..." required
                    class="flex-1 min-w-0 bg-gray-100 border-none rounded-full px-5 py-2.5 text-sm focus:ring-2 focus:ring-blue-400">

                <button type="submit"
                    class="flex-shrink-0 w-10 h-10 bg-blue-500 hover:bg-blue-600 text-white rounded-full flex items-center justify-center shadow-md transition-transform active:scale-90">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 rotate-45 -translate-x-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
