...
<head>
    ...
    {{-- ここを修正：$titleがあれば表示、なければ config の値を出す --}}
    <title>{{ $title ?? config('app.name', 'CAPSULE LINK') }}</title>
    {{-- ここを追加：$descriptionがあれば表示、なければデフォルトを出す --}}
    <meta name="description" content="{{ $description ?? '宮崎のガチャガチャ交換所「CAPSULE LINK」。送料なしの直接手渡しで被ったカプセルトイを交換しましょう。' }}">
